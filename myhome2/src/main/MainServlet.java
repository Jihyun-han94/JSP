...
public class MainServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 세션(Session)
		 *		쿠키처럼 서버가 클레이언트에 세션 정보를 저장을 하나 이 세션 정보는 암호화가된 세션ID이고 
		 *		이 세션 ID 에는 어떠한 개인정보도 포함되어 있지 않다. (쿠키의 단점 보완)
		 *		이 세션 ID 가 서버에 전달이 되면 서버에 저장되어 있는 동일한 세션 ID의 정보를 획득하여 
		 *		이전 사용자에 대한  기록을 확인 할 수 있다. 
		 *		즉, 민감한 데이터들은 서버에 저장되어 있는 것이다. (쿠키는 클라이언트에 다 저장되어 있음.)
		 *
		 * */
